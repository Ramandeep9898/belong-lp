  ...
  return (
    <div className="px-9 md:py-[120px] md:px-0 w-full relative z-[4000]  flex flex-col pt-12 pb-36 justify-center items-center bg-[#FBF8F6]">
      <div className="flex flex-col justify-center items-center">
        <p className="mb-4  font-garnett text-[#1D1D1D]">{FEATURES_SECTION_HEADING}</p>
      ...
      <div className="flex w-[300px] md:w-[800px] flex-wrap  gap-6 md:gap-0 justify-center items-center">
        {FEATURES_INFO.map((item, index, array) => (
          <FeatureCard key={index} cardInfo={item} index={index} array={array}/>
        ))}
      </div>
    </div>
  );
